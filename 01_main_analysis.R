...
nhanes_cleaned <- nhanes_cleaned %>%
  select(-SDDSRVYR, -RIDSTATR, -RIDAGEMN, -RIDEXMON, -RIDEXAGM, -DMDHRGND, -DMDHRAGZ, -DMDHREDZ, -DMDHRMAZ, -DMDHSEDZ
         , -OCQ210, -OCQ215, -OCQ383, -OCQ180, -WTMEC2YR, -RIDRETH1, -DMQMILIZ, -DMDMARTZ, -RIDEXPRG, -HIQ032A, -HIQ032B
         , -HIQ032C, -HIQ032D, -HIQ032E, -HIQ032F, -HIQ032H, -HIQ032I, -HIQ210, -DMDHHSIZ, -HUQ030, -HUQ042, -HUQ055
         , -HUQ090 , -HOD051, -INQ300, -IND310, -INDFMMPC)
...
nhanes_cleaned <- nhanes_cleaned %>%
  mutate(
    health_insurance = case_when(
      ...
      health_insurance %in% c(7, 9) | is.na(health_insurance) ~ NA_integer_
    )
  )
...
nhanes_cleaned <- nhanes_cleaned %>%
  mutate(
    years_in_US = na_if(years_in_US, 77),
...
vars_to_test <- nhanes_cleaned %>% 
  select(gender, age, race, years_in_US, education, income_ratio_log,
         health_insurance, general_health, occupation, immigrant_status)
...
test_missingness_for_var <- function(var, data, predictors) {
  ...
  form <- as.formula(
    paste0("miss_", var, " ~ ", paste(preds, collapse = " + "))
  )
  
  data %>%
    mutate(!!paste0("miss_", var) := as.integer(is.na(.data[[var]]))) %>%
    glm(formula = form, data = ., family = binomial()) %>%
    tidy() %>%
...
nhanes_cleaned <- nhanes_cleaned %>%
  ...
  mutate(
    ...
    phq9_total = if_else(
      phq_items_answered >= 7,
      sum(c_across(DPQ010:DPQ090), na.rm = TRUE),
      NA_real_
    )
  ) %>%
  ungroup()
...
num_vars <- num_vars[, !names(num_vars) %in% c("DPQ010", "DPQ020", "income_ratio",
                                               "DPQ030", "DPQ040", "DPQ050", "DPQ060", 
                                               "DPQ070", "DPQ080", "DPQ090", "DPQ100", 
                                               "INDFMMPI", "SEQN", "phq_items_answered",
                                               "WTINT2YR", "SDMVSTRA", "SDMVPSU")]       
...
mean_diff <- nhanes_demo %>%
  ...
  group_by(variable) %>%
  summarise(
    diff = mean(value[immigrant_status_fac == "Immigrant"], na.rm = TRUE) -
...
cont_es <- nhanes_demo %>%
  ...
  group_by(variable) %>%
  summarise(
    d = effectsize::cohens_d(value ~ immigrant_status_fac, pooled_sd = TRUE, na.rm = TRUE)$Cohens_d,
    .groups = "drop"
  ) %>%
  mutate(
...
cat_es <- lapply(cat_vars, function(v) {
  ...
  tibble(variable = v, effect_size = ifelse(is.na(V), NA_character_, sprintf("V=%.2f", V)))
}) %>%
  bind_rows()
...
demographic_table_base <- nhanes_demo %>%
  select(
    ...
    age, income_ratio, years_in_US, phq9_total,
    gender_fac, race_fac, education_fac, occupation_fac,
    health_insurance_fac, general_health_fac
  ) %>%
  tbl_summary(
...
demographic_table <- demographic_table_base %>%
  modify_table_body(~ .x %>%
                      ...
                      group_by(variable) %>%
                      mutate(
                        effect_size = if_else(row_number() == 1, effect_size, NA_character_)
                      ) %>%
                      ungroup() %>%
  ...
  modify_header(
    comparison  ~ "Comparison statistic",
    p.value     ~ "p value",
...
fig_numeric <- ggplot(num_long, aes(x = Value, linetype = Split)) +
  ...
  labs(
    title = "Distribution comparison (Development vs Test): Numeric predictors and outcome",
    x = NULL,
    y = "Density"
  ) +
  apa_theme
...
fig_categorical <- ggplot(cat_props, aes(x = Level, y = prop, fill = Split)) +
  ...
  labs(
    title = "Distribution comparison (Development vs Test): Categorical predictors",
    x = NULL,
    y = "Proportion"
  ) +
  apa_theme +
...
ggsave("outputs/Figure_Distributions_Numeric_Dev_vs_Test.png", fig_numeric, width = 12, height = 8, dpi = 300)
ggsave("outputs/Figure_Distributions_Categorical_Dev_vs_Test.png", fig_categorical, width = 12, height = 10, dpi = 300)
...
cal_fit <- cal_all %>%
  group_by(Model) %>%
  summarise(
    intercept = coef(lm(mean_obs ~ mean_pred))[1],
  ...
  mutate(
    ...
    label = paste0(
      "Intercept = ", formatC(intercept, format = "f", digits = 2),
...
p_cal <- ggplot(cal_all, aes(x = mean_pred, y = mean_obs)) +
  ...
  labs(
    ...
    x = "Mean predicted PHQ-9",
    y = "Mean observed PHQ-9"
  ) +
  apa_theme + theme(
...
box_labs <- pred_all %>%
  ...
  mutate(
    label = paste0(
      "Median = ", formatC(med, format = "f", digits = 2),
      "\nN = ", n
    ),
    y_pos = q3 + 1.5,
...
p_err_box <- ggplot(pred_all, aes(x = Model, y = abs_resid)) +
  ...
  geom_text(
    ...
    aes(x = x_pos, y = y_pos, label = label),
    inherit.aes = FALSE,
    hjust = 0,   # left align, so it reads nicely to the right
    size = 3
  ) +
...
pred_test <- bind_rows(
  tibble(Model = "Linear Regression",
...
boot_ci <- function(y, yhat, metric_fun, B = 2000, conf = 0.95) {
  n <- length(y)
...
perf_ci_apa <- perf_ci_table %>%
  ...
  tab_header(
    title = md("**Table X**"),
    subtitle = md("*Test-set performance metrics with 95% bootstrap confidence intervals (B = 2000)*")
  ) %>%
  cols_label(
    Metric = "Metric",
...
test_df3 <- nhanes_test %>%
  mutate(
    gender_lab = factor(gender, levels = c(1,2), labels = c("Male", "Female")),
    imm_lab    = factor(immigrant_status, levels = c(1,2), labels = c("Native-born", "Immigrant")),
    race_lab   = factor(as.character(race)),  # keep as character codes (1,2,3,4,6,7)
    age_bin    = cut(
      age,
...
nhanes_cleaned %>%
  ...
  labs(
    x = "PHQ-9 total score",
    y = "Density",
    linetype = "Immigration status"
  ) +
  theme_classic(base_size = 12) +
  theme(
    plot.title = element_blank(),        # APA: no title in figure
    legend.position = "top",
...
ggsave(
  ...
  plot     = last_plot(),
  width    = 6.5,    # APA-recommended figure width (inches)
  height   = 4.5,    # balanced height for density plots
  dpi      = 300,
  bg       = "white"
)
